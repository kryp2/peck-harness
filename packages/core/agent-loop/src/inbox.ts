...
export const inboxProjectionDefinition = {
  ...
  apply(state: InboxState, event) {
    ...
    // Repair before validating: logs written before the full message
    // representation stored raw prompt text in `inserted`, and every consumer
    // reads `id`, `content`, and `source` unconditionally.
    const splice = {
      ...event.data,
      inserted: (event.data.inserted as PersistedInboxEntry[])
        .map((entry, index) => repairPersistedEntry(entry, event.seq, index)),
    }
    try {
