...
for (const [file, jobIds] of [['release.yml', ['dependencies', 'pack']], ['release-vendor.yml', ['pack']]] as const) {
  describe(file, () => {
    ...
    it('preserves the logical jobs, rehearsal events and read-only permission', () => {
      ...
      // FORK POLICY (deepseek-harness PR #12, owner decision 2026-08-24): pack
      // stays budgeted — tag push + manual dispatch only, never pull_request or
      // master push. The routing, permission, concurrency, and command
      // assertions below stay verbatim; only the trigger posture differs here.
      expect(release.on).toEqual(file === 'release.yml'
