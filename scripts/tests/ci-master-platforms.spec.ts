...
describe('master-only platform scheduling', () => {
  ...
  it('runs all three deferred carriers on master pushes with fail-loud API credentials', () => {
    ...
    // ci-master is dispatch-only on this fork — an automatic master push would
    // only queue runs for runners that do not exist here. The carriers keep
    // their push-gated `if:` (harmless under dispatch-only: they evaluate
    // false on a manual dispatch), so they stay ready if the triggers are
    // ever re-owned. Only the `on:` posture differs here.
