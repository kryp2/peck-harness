...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-brand-peck': { kind: 'none', reason: 'Browser-side presentation occupants and theme-token override layer; registers nothing model-facing.' },
  ...
  'packages/bundle/peck': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; the preset it selects and web-runtime\'s productName own every model-facing effect it contributes.' },
  ...
  'packages/session/session-usage': { kind: 'none', reason: 'The usageByRoute unit folds already-logged request config and usage into a client-facing read model and registers nothing model-facing.' },
  ...
  'packages/interaction/telegram-answerer': { kind: 'none', reason: 'The answerer posts outward to Telegram and registers no prompt, tool, or session event; dsh-tool-ask-user owns the question flow.' },
