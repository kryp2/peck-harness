...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    for (const [jobName, job] of [['windows-build', windowsBuild], ['windows-coverage', windowsCoverage], ['windows-native-tests', windowsNativeTests], ['windows-observational', windowsObservational]] as const) {
      ...
      // hosted fallback is GitHub's `windows-latest` — the upstream
      ...
      expect(job['runs-on']).not.toContain('dsh-windows-')
  ...
  it('keeps ci-master dispatch-only, cancelling each superseded dispatch, with inert master-push guards intact', () => {
    ...
    // Pin the post-merge runtime, Wine, and standby inventory, normalized for
    // the fork policy: the guards are inert here (dispatch can never satisfy
    // them), but the parked jobs must still be exactly these four — adding a
    // job with a dispatch-reachable condition starts (and bills) work on
    // every manual dispatch.
    const NOT_DISPATCH_REACHABLE = new Set([
      "github.event_name == 'workflow_dispatch' && inputs.suite == 'larger-runner-benchmark'",
      "github.event_name == 'workflow_dispatch' && inputs.suite == 'consolidated-runner-benchmark'",
    ])
    const dispatchReachable = Object.entries(workflow.jobs)
      .filter(([, job]) => {
        ...
        return !NOT_DISPATCH_REACHABLE.has(job.if.trim())
          && job.if.trim() !== MASTER_PUSH_GUARD
      })
      .map(([name]) => name)
      .sort()
    expect(
      dispatchReachable,
      'only the two benchmark suites may start on a manual dispatch',
    ).toEqual([])
...
// Fork posture (carried through the 2026-09-07 upstream sync, owner decision
// 2026-08-24): the issue-lifecycle/issue-policy workflows target the upstream
// org's Project board and Issue App (deepseek-harness org, DSH_ISSUE_APP_*
// credentials this fork does not operate). They are merged verbatim so the
// fork keeps the workflow sources, but they must stay unwired here — re-owning
// them to this fork's org or adding org secrets would arm an unreviewed
// write-capable automation. Do not "fix" the org names below.
...
describe('npm release workflows', () => {
  ...
  it('runs dependency policy and npm layout checks in the DSH release workflow', () => {
    ...
    // FORK POLICY (deepseek-harness PR #12, owner decision 2026-08-24): the
    // commands stay, the triggers do not — this fork proves the pack proof at
    // tag/dispatch time only, never on pull_request or master push.
