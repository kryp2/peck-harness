...
| Peck composition layer (product bundle, client brand package, agent preset) | peck | `apps/cli/config/agent-presets/peck/**`, `packages/bundle/peck/**`, `packages/client/ui-brand-peck/**` | Wholly peck-owned path space: the product composition lives here so generic presets, bundles, and brand packages stay upstream-neutral; offered upstream only if upstream adopts a product-composition seam | bundle and client-package vitest suites; `verify-cordis-config` plane-separation and dependency checks |
