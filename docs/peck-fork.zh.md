...
| peck 包的工作区注册 | upstream-shared | `pnpm-lock.yaml`、`tsconfig.base.json`、`tsconfig.client.json`、`tsconfig.host.json`、`scripts/gen-cordis-catalog.ts`、`scripts/verify-package-readme-model-experience.ts` | 随 peck 包成员变化再生成或重放；绝不单独上游化 | `constraints`、`verify-runtime-closure`、目录新鲜度门禁 |
