...
Issue 生命周期仍不订阅 `pull_request.ready_for_review`；两条事件命令均不依赖该动作。Issue 策略保留 `ready_for_review`，因为人工提交的 PR 进入评审时，该工作流负责执行必需检查门禁。本 fork 不携带 `issue-lifecycle.yml` 与 `issue-policy.yml`；以上描述的是本笔记记录时针对的上游自动化。
