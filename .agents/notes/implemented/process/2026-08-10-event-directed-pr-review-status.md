...
Issue lifecycle remains unsubscribed from `pull_request.ready_for_review`; neither event command depends on that action. Issue policy retains `ready_for_review` because it owns required-check enforcement when a human pull request enters review. This fork carries neither `issue-lifecycle.yml` nor `issue-policy.yml`; those statements describe the upstream automation this note was recorded against.
